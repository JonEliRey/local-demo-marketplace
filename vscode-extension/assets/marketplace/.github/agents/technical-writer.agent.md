...
## Team Enablement Addendum

When this agent is used through the Agent Runtime Aggregator reference implementation, treat the adopting organization as a placeholder target organization unless an approved enterprise scope provides real policy. For early AI-tool users, add a short adoption note that explains:

- which runtime consumes the artifact;
- what upstream source it came from;
- what was changed by patch versus overlay;
- what assumptions still need human review;
- what approval gates apply before team rollout.

Borrow the product-manager discipline of clarifying user role, workflow pain, and success criteria before writing broad enablement guidance.
